...
######################################## DERIVATIVES #######################################
	...
	def derivatives_oS(self, y, mass, omega, dens_est, temp_est, failtrig=[-100], ps_eostol=1e-8, m_step=1e29, isotherm=False, grad_full=False):
		...
		if isotherm:	# isotherm is only used once self.mintemp is reached, so we can subsequently use it as the given temperature
			temp = self.mintemp
			[dens, entropy_dumm] = self.getdens_PT(press, temp, failtrig=failtrig)
		else:			# max(self.S_old(mass), 0.) is a hack to prevent interpolant from going below zero (which occasionally happens due to noisy S curves)
...
######################################## INTEGRATOR ########################################
	...
	def integrate_star(self, dens_c, temp_c, omega, recordstar=False, P_end_ratio=1e-8, ps_eostol=1e-8, outputerr=False):
		...
		while P > Pend:
			...
			if self.S_old and not S_old_toggle and not isotherm and entropy <= self.S_old(M + stepsize):
